...
to_real(x::IntegerAst; ctx::Context=global_ctx()) = AppAst{Real}(mk_int2real(ctx, x))
to_int(x::RealAst; ctx::Context=global_ctx()) = AppAst{Integer}(mk_real2int(ctx, x))
...
for (op,func) in real_real_bool
  ...
  @eval ($op)(x::RealAst, y::IntegerAst; ctx::Context = global_ctx()) =
    ($op)(x, to_real(y; ctx=ctx); ctx=ctx)

  @eval ($op)(x::IntegerAst, y::RealAst; ctx::Context = global_ctx()) =
  ...
  @eval ($op)(x::LeafReal, y::RealAst; ctx::Context = global_ctx()) =
    AppAst{Bool}($func(ctx, NumeralAst(Real, x), y))

  @eval ($op)(x::IntegerAst, y::LeafInteger; ctx::Context = global_ctx()) =
    AppAst{Bool}($func(ctx, x, NumeralAst(Integer, y)))

  @eval ($op)(x::LeafInteger, y::IntegerAst; ctx::Context = global_ctx()) =
    AppAst{Bool}($func(ctx, NumeralAst(Integer, x), y))
end

distinct(x::RealAst...; ctx::Context = global_ctx()) =
...
for (op, func) in real_real_real
  ...
  @eval ($op)(x::LeafInteger, y::RealAst; ctx::Context = global_ctx()) =
    ($op)(NumeralAst(Integer, x), y; ctx=ctx)
end

# Unary Operators
(-)(x::RealAst; ctx::Context=global_ctx()) = AppAst{Real}(mk_unary_minus(ctx, x))
(-)(x::IntegerAst; ctx::Context=global_ctx()) = AppAst{Integer}(mk_unary_minus(ctx, x))
...
ifelse(a::BoolAst, b::BoolAst, c::BoolAst; ctx::Context = global_ctx()) =
  AppAst{Bool}(mk_ite(ctx, a, b, c))

ifelse(a::BoolAst, b::RealAst, c::RealAst; ctx::Context = global_ctx()) =
  AppAst{Real}(mk_ite(ctx, a, b, c))

ifelse(a::BoolAst, b::IntegerAst, c::RealAst; ctx::Context = global_ctx()) =
  AppAst{Real}(mk_ite(ctx, a, to_real(b), c))

ifelse(a::BoolAst, b::RealAst, c::IntegerAst; ctx::Context = global_ctx()) =
  AppAst{Real}(mk_ite(ctx, a, b, to_real(c)))

ifelse(a::BoolAst, b::IntegerAst, c::IntegerAst; ctx::Context = global_ctx()) =
  AppAst{Integer}(mk_ite(ctx, a, b, c))

"Is this real number an integer"
is_int(a::RealAst; ctx::Context = global_ctx()) =
