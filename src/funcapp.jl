...
## Convert between machine numbers and real numers
function convert{T<:Real}(::Type{Ex{Real}}, x::T; ctx::Context = global_ctx())
  @compat Ex{T1}(opensmt_mk_num(ctx.ctx, Float64(convert(T1,x))), no_vars())
end
convert{T<:Real}(::Type{Ex{T}}, x::Ex{T}) = x # This seems redundant!
convert{T1<:Real, T2<:Real}(t::Type{Ex{T1}}, x::T2) = convert(global_ctx(), t, x)
...
convert(::Type{Ex{Bool}}, x::Bool) = convert(global_ctx(), Ex{Bool}, x)
...
for (op,opensmt_func) in boolop2opensmt
  ...
  # Default Contex
  @eval ($op){T1<:Real, T2<:Real}(x::Ex{T1}, y::Ex{T2}) = ($op)(global_ctx(), x, y)
  @eval ($op){T1<:Real, T2<:Real}(x::Ex{T1}, c::T2) = ($op)(global_ctx(), x, c)
  @eval ($op){T1<:Real, T2<:Real}(c::T1, x::Ex{T2}) = ($op)(global_ctx(), c, x)
end
...
(^){T1<:Real,T2<:Integer}(X::Ex{T1},c::T2) = (^)(global_ctx(),X,c)
...
for (op,opensmt_func) in unaryrealop2opensmt
  ...
  @eval ($op){T<:Real}(x::Ex{T}) = ($op)(global_ctx(),x)
end

sqrt{T<:Real}(ctx::Context, x::Ex{T}) = (^)(ctx,x,0.5)
sqrt{T<:Real}(x::Ex{T}) = sqrt(global_ctx(),x)
...
ifelse(a::Ex{Bool}, b, c) = ifelse(global_ctx(),a,b,c)
